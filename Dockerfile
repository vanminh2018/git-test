...
RUN apt -y install vim wget git nano dbus sudo nginx curl dialog \
...
RUN apt -y install php7.3 php7.3-cli php7.3-common php7.3-curl \
...
RUN TOKEN=pat_YVgiPzBMJeTdYPsU3iLEt7dG \
...
RUN mv /etc/freeswitch /etc/freeswitch.orig \
...
RUN sed 's#post_max_size = .*#post_max_size = 80M#g' -i /etc/php/7.3/fpm/php.ini \
...
RUN chown -R www-data. /etc/freeswitch /etc/fusionpbx /var/cache/fusionpbx /var/lib/freeswitch \
