...
//page title and description
	...
	//basic search of call detail records
		if (permission_exists('xml_cdr_search')) {
			...
			echo "		<td class='vtable' align='left'>\n";
			echo "			<select class='formfld' name='caller_extension_uuid' style='width:100%'>\n";
			echo "				<option value=''></option>";
			...
			echo "		</td>\n";
			echo "		<td class='vtable' align='left' style='white-space: nowrap;'>\n";
			echo "			<input type='text' class='formfld' name='duration' id='duration' value='".escape($duration)."'>\n";
			echo "		</td>\n";
			echo "	</tr>\n";
			echo "</table>\n";
			echo "</td>";
...
//show the results
	...
	echo "<b>Total CDRs: <span>$total_cdrs</span></b>\n";
	...
	//column headings
		...
		echo th_order_by('callrate', $text['label-callrate'], $order_by, $order, null, null, $param);
	...
	if ($result_count > 0) {
		...
		foreach($result as $index => $row) {
			...
			//if call cancelled, show the ring time, not the bill time.
				...
				$seconds = ($row['hangup_cause']=="ORIGINATOR_CANCEL" || $row['hangup_cause'] == 'NORMAL_CLEARING') ? $row['duration'] : round(($row['billmsec'] / 1000), 0, PHP_ROUND_HALF_UP);
			...
			//hangup cause/call result
				...
				// Callrate
				echo "	<td valign='top' class='".$row_style[$c]."' nowrap='nowrap'>".ucwords(escape($callrate))."</td>\n";
			//control icons
