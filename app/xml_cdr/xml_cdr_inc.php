...
// 
    $start_stamp_begin = CheckTime($_REQUEST["start_stamp_begin"]);
    $start_stamp_end = CheckTime($_REQUEST["start_stamp_end"]);
    $answer_stamp_begin = CheckTime($_REQUEST["answer_stamp_begin"]);
    $answer_stamp_end = CheckTime($_REQUEST["answer_stamp_end"]);
    $end_stamp_begin = CheckTime($_REQUEST["end_stamp_begin"]);
    $end_stamp_end = CheckTime($_REQUEST["end_stamp_end"]);
    $start_epoch = CheckTime($_REQUEST["start_epoch"]);
    $stop_epoch = CheckTime($_REQUEST["stop_epoch"]);
    // 
...
if (!isset($start_stamp_begin))
{
    $start_stamp_begin = date("Y-m-d ");
}
else
{
    $start_stamp_begin = $start_stamp_begin;
}
if (!isset($start_stamp_end))
{
    $start_stamp_end = date("Y-m-d ") . "23:59";
}
else
{
    $start_stamp_end = $start_stamp_end;
}
  
if (strlen($caller_id_number) >0 )
{
    $sql_where_ands[] = "caller_id_number LIKE '%" . $caller_id_number . "%' ";
}
...
{
    $sql_query_status .= " WHEN (answer_stamp is null and bridge_uuid is null and sip_hangup_disposition <> 'send_refuse') THEN 'BUSY' ";
}
else if ($direction == 'outbound')
{
    $sql_query_status .= " WHEN (answer_stamp is null and bridge_uuid is not null) THEN 'BUSY' ";
}
else
{
    $sql_query_status .= "
...
$sql_query_status .= " WHEN (answer_stamp is null and bridge_uuid is null and billsec = 0 and sip_hangup_disposition = 'send_refuse') THEN 'FAILED' ";
$sql_query_status .= " ELSE 'UNKNOWN' ";
$sql_query_status .= " END AS Status ";
...
if (!permission_exists('xml_cdr_domain'))
{
    ...
    { // extensions are assigned to this user
        ...
        if ($caller_id_number != '' && $destination_number != '' && array_search($caller_id_number, $user_extensions) === false && array_search($destination_number, $user_extensions) === false)
        {
        ...
        if ($caller_id_number == '')
        { // if source criteria is blank, then restrict to assigned ext
...
// $param .= "&start_stamp_begin=" . escape($start_stamp_begin . "%3A00");
// $param .= "&start_stamp_end=" . escape($start_stamp_end . "%3A00");
...
$count_sql = "select count(c.domain_uuid) as total_cdrs ";
$sql = "select \n";
$sql .= "c.domain_uuid, \n";
$sql .= "CASE WHEN cr.keypress IS NULL THEN '0' ELSE cr.keypress END AS callrate, \n";
$sql .= "e.extension, \n";
$sql .= "c.start_stamp, \n";
$sql .= "c.end_stamp, \n";
$sql .= "c.start_epoch, \n";
...
$sql .= " , $sql_query_status \n";
$sql .= "from v_xml_cdr as c \n";
$count_sql .= "from v_xml_cdr as c \n";
$sql .= "left join v_extensions as e on e.extension_uuid = c.extension_uuid ";
$sql .= "left join call_rate cr on c.domain_uuid = cr.domain_uuid and c.xml_cdr_uuid  = cr.call_id ";
$count_sql .= "left join v_extensions as e on e.extension_uuid = c.extension_uuid ";
$sql .= "inner join v_domains as d on d.domain_uuid = c.domain_uuid ";
$count_sql .= "inner join v_domains as d on d.domain_uuid = c.domain_uuid ";
if ($_REQUEST['show'] == "all" && permission_exists('xml_cdr_all'))
{
    if ($sql_where)
    {
...
else
{
    ...
    $count_sql .= "where c.domain_uuid = '" . $domain_uuid . "' \n";
}
$sql .= $sql_where;
$count_sql .= $sql_where;
if (strlen($order_by) > 0)
...
$count_sql = str_replace("where and", "where", $count_sql);
...
$database->select($count_sql);
$total_cdrs_resp = $database->result;
$total_cdrs = $total_cdrs_resp[0]["total_cdrs"];
unset($database);
