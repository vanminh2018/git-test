								...
								// build voicemail
									if ($voicemail_id !== NULL) {
										...
										//if voicemail_uuid does not exist then get a new uuid
											...
											if ($voicemail_enabled == "false" || $voicemail_enabled == false || $voicemail_enabled == "") {
												$sql_del = "delete from v_voicemails where voicemail_uuid='".$voicemail_uuid."'";
												$prep_statement_del = $db->prepare(check_sql($sql_del));
												$prep_statement_del->execute();
											}
											if ($voicemail_enabled != "false" && $voicemail_enabled != false && $voicemail_enabled != "")
											{
...
//begin the page content
	...
	if (permission_exists('voicemail_edit') && is_dir($_SERVER["DOCUMENT_ROOT"].PROJECT_PATH.'/app/voicemails')) {
		...
		if ($voicemail_enabled == "false" || $voicemail_enabled == "") {
