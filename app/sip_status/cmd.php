	...
	if ($fp) {
		//if reloadxml then run reloadacl, reloadxml and rescan the external profile for new gateways
			if ($command == "api reloadxml") {
				//reloadxml
					if ($command == "api reloadxml") {
                                                // Minh Tuan send a custom event
                                                $event = "sendevent CUSTOM\n";
                                                $event .= "Event-Name: CUSTOM\n";
                                                $event .= "Event-Subclass: fusion::reloadxml\n";
                                                $event .= "API-Command: reloadxml\n";
		...
		//reloadacl
			if ($command == "api reloadacl") {
                                ...
                                $event = "sendevent CUSTOM\n";
                                $event .= "Event-Name: CUSTOM\n";
                                $event .= "Event-Subclass: fusion::reloadacl\n";
                                $event .= "API-Command: reloadacl\n";
                                event_socket_request($fp, $event);
				messages::add(rtrim(event_socket_request($fp, $command)), 'alert');
				unset($command);
			}
