...
if (count($_POST) > 0 && strlen($_POST["persistformvar"]) == 0) {
	...
	$msg = '';
	$logo_path = "/themes/default/images/logo.png";
	if (isset($_FILES['logo_upload'])){
		$allow_extension = ['jpg','png','jpeg'];
		if ($_FILES['logo_upload']['size'] != 0){
			$upload_filename = $_FILES['logo_upload']['name'];
			$upload_file_extension = strtolower(pathinfo($upload_filename,PATHINFO_EXTENSION));
			if (!in_array($upload_file_extension,$allow_extension))
			{
				$msg=$msg.$upload_file_extension." is not allowed<br>\n";
			}
			else if ($_FILES['logo_upload']['size']> 5000000){
				$msg=$msg."Upload image must smaller than 5MB<br>\n";
			}
			else if (getimagesize($_FILES["logo_upload"]["tmp_name"])==false){
				$msg=$msg."File is not an image<br>\n";
			}
			else{
				$date = new DateTime();
				$logo_dir = "/themes/default/logo/".$domain_uuid;
				$logo_path = $logo_dir . "/". $date->getTimestamp()."_".$_FILES['logo_upload']['name'];
				$save_dir = $_SERVER["PROJECT_ROOT"]."/". $logo_dir;
				$save_path = $_SERVER["PROJECT_ROOT"]."/". $logo_path;
				if (is_dir($save_dir) == false)
				{
					mkdir($save_dir);
				}
				if (!move_uploaded_file($_FILES["logo_upload"]["tmp_name"],$save_path)) {
					die("Cannot upload logo");
				}
			}
		}
	}
	...
	//add or update the database
		if ($_POST["persistformvar"] != "true") {
			...
			if ($action == "update" && permission_exists('domain_edit')) {
				...
				if ($logo_path != "/themes/default/images/logo.png"){
					$sql .= "domain_enabled = '".$logo_path."', ";
				}
