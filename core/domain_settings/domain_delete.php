...
// Don't allow delete current domain
if (strlen($id) > 0 && $_SESSION['domain_uuid'] != $id) {
	...
	//delete the directories
		...
		// Update session for domain
		if($_SESSION['domains']) {
			foreach($_SESSION['domains'] as $k => $node) {
				if($k == $id && $_SESSION['domain_uuid'] != $id) {
					unset($_SESSION['domains'][$k]);
				}
			}
		}
