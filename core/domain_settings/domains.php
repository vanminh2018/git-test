	...
	if (strlen(check_str($_GET["domain_uuid"])) > 0 && check_str($_GET["domain_change"]) == "true") {
		if (permission_exists('domain_select')) {
			...
			//update the domain session variables
				...
				$_SESSION['domain_logo'] = $_SESSION['domains'][$domain_uuid]['domain_logo'];
				if ($_SESSION['domain_logo']=='')
				{
					$_SESSION['domain_logo'] = '/themes/default/images/logo.png';
				}
				// var_dump($_SESSION['domains']);
				// die($_SESSION['domains']);
	...
	if (count($domains) > 0) {
		foreach ($domains as $domain_uuid => $domain) {
			...
			if (permission_exists('domain_delete')) {
				if ($_SESSION["groups"][0]["domain_uuid"] != $domain_uuid && count($_SESSION['domains']) > 1 && $_SESSION['domain_uuid'] != $domain_uuid) {
